...
2. **The quant recipe is fixed and validated: quantize the decoder FFN/expert GEMMs only.** Keep **high precision (f16/f32)**: the entire vision tower, projector, `embed_tokens`, the **MoE router gate**, and **ALL norms**. Quantizing the vision encoder wrecks OCR (both prior-art quants keep it). int8 on attention `q/k/v/o` and on `lm_head` go *beyond* the validated set — gate them behind a measured-CER kill-switch (OQ-14), never assume they are lossless.
...
4. **The wedge we are actually building:** frankentorch's int8 dot is a per-output-element SDOT/VNNI dot and is **still ~1.5–2.4× behind ONNX/MLAS**. A **tiled, register-blocked SMMLA + AVX-512-VNNI/AMX GEMM with offline weight pre-packing** (and int4-group) is the unbuilt lever that closes that gap — it does not exist anywhere yet, and building it is the reason this project exists.
...
7. **R-SWA: constant MEMORY, not constant per-token COMPUTE.** The reference block `m` grows ~256 tokens/page and is attended every step, so per-token attention is `O(m+128)`. Multi-page decode is **cross-page dependent** (OQ-13) — do not assume "multi-page = sum of single-page parses."
