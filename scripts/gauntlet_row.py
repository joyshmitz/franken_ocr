...
def load_json(path: str, want_schema: str | None = None) -> dict:
    ...
    if want_schema and doc.get("schema") != want_schema:
        raise RowError(f"{path}: expected schema {want_schema}, got {doc.get('schema')!r}")
    return doc
...
def validate_inputs(
    focr: dict, ref: dict, stage: str, *, allow_synthetic: bool
) -> tuple[dict, dict]:
    for name, doc in (("focr", focr), ("reference", ref)):
        if doc.get("synthetic") and not allow_synthetic:
            raise RowError(f"{name} input is stamped synthetic — self-test data can never land")
    ...
    for side, rec in (("focr", frec), ("reference", rrec)):
        cv = rec.get("cv_pct")
        if cv is None and rec.get("n", 0) < 2:
            raise RowError(f"{side} {stage}: single sample — best-of-N with cv% is mandatory")
        if cv is not None and cv > MAX_CV_PCT:
            raise RowError(
                f"{side} {stage}: cv_pct {cv} > {MAX_CV_PCT} — noise is ineligible to land"
            )
        if not rec.get("precision"):
            raise RowError(f"{side} {stage}: missing precision annotation")
        if not rec.get("best_ms"):
            raise RowError(f"{side} {stage}: missing best_ms")
    ...
    if proof.get("torch_num_threads") != rrec.get("threads"):
        raise RowError("reference record lacks a matching torch thread proof — rejected")
    ...
    if focr.get("stdout_identical_across_runs") is not True:
        raise RowError("focr stdout differed across runs — nondeterministic evidence rejected")
...
def build_row(
    *,
    ...
    frec, rrec = validate_inputs(focr, ref, stage, allow_synthetic=allow_synthetic)
    ...
    if not kill_cell:
        raise RowError("focr run meta carries no FOCR_* env evidence (kill-switch state)")
    ...
    cells = {
        ...
        "notes": notes or f"best-of-{frec['n']} (cv {frec['cv_pct']}% / ref {rrec['cv_pct']}%)",
    }
...
def write_bundle(
    ...
    *,
    ...
    for src in (focr_path, ref_path, roofline_path):
        shutil.copy2(src, os.path.join(evidence_dir, os.path.basename(src)))
    ...
    with open(os.path.join(evidence_dir, "row.json"), "w", encoding="utf-8") as f:
        json.dump(
            {
                "schema": "focr-gauntlet-row/v1",
                "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                "inputs": {
                    "focr_stages": {"path": focr_path, "sha256": sha256_file(focr_path)},
                    "ref_stages": {"path": ref_path, "sha256": sha256_file(ref_path)},
                    "roofline": {"path": roofline_path, "sha256": sha256_file(roofline_path)},
                },
                "rows": rows,
            },
            f,
        ...
        f.write("\n")
    with open(os.path.join(evidence_dir, "PERF_LEDGER_ROW.md"), "w", encoding="utf-8") as f:
        for row in rows:
...
def insert_rows(ledger_text: str, rows_md: list[str]) -> str:
    ...
    for i, line in enumerate(lines):
        if line.startswith("| date | claim_id | evidence_id |"):
            header_idx = i
            break
    if header_idx is None or header_idx + 1 >= len(lines) or "---" not in lines[header_idx + 1]:
        raise RowError("PERF_LEDGER.md ratio-table header not found; cannot insert row")
    insert_at = header_idx + 2
    return "".join(lines[:insert_at] + [md + "\n" for md in rows_md] + lines[insert_at:])


def shadow_check(rows_md: list[str], evidence_dir: str, evidence_id: str) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="focr-gauntlet-row-") as shadow:
        ...
        for name in ("NEGATIVE_EVIDENCE.md", "DISCREPANCIES.md"):
            shutil.copy2(os.path.join(ROOT, "docs", name), os.path.join(shadow, "docs", name))
        with open(LEDGER, encoding="utf-8") as f:
            ledger_text = f.read()
        with open(os.path.join(shadow, "docs", "PERF_LEDGER.md"), "w", encoding="utf-8") as f:
...
def run(args: argparse.Namespace) -> int:
    ...
    stages = args.stage or [
        ...
        if stage_record(focr, s) and stage_record(ref, s)
    ...
    evidence_dir = args.evidence_dir or os.path.join(ROOT, "artifacts", "perf", "bd-re8.17", claim)
    evidence_id = os.path.relpath(os.path.abspath(evidence_dir), ROOT)
    if not evidence_id.startswith(os.path.join("artifacts", "perf")):
        raise RowError(f"evidence dir must live under artifacts/perf/, got {evidence_id!r}")
    ...
    if not ok:
        ...
        print("ERROR: candidate row fails scripts/check_ledgers.py — not emitted", file=sys.stderr)
        return 1
    ...
    if args.apply:
        ...
        if not applied:
            ...
            print("ERROR: in-place apply failed validation and was reverted", file=sys.stderr)
            return 1
...
def _synthetic_inputs(tmp: str) -> tuple[str, str, str, dict]:
    ...
    def rec(stage: str, samples: list[float], source: str, **extra: object) -> dict:
        return {
            ...
            "cv_pct": round(statistics.stdev(samples) / statistics.fmean(samples) * 100, 3),
            "n": len(samples),
            "warmup_discarded": 1,
            "threads": 8,
    ...
    focr = {
        ...
        "stages": [
            rec("decode_per_token", [10.0, 10.2, 10.1], "focr", tokens=600, tokens_consistent=True)
        ],
        "synthetic": True,
    }
    ref = {
    ...
    roofline = {
        ...
        "floors": [{"stage": "decode_per_token", "floor_kind": "memory", "floor_ms": 4.953}],
        "synthetic": True,
    }
    paths = []
    for name, doc in (("focr.json", focr), ("ref.json", ref), ("roofline.json", roofline)):
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="focr-gauntlet-row-selftest-") as tmp:
        ...
        kwargs = dict(
            ...
            correctness_proof="synthetic self-test — not evidence",
            model_commit=MODEL_COMMIT,
            notes="",
        )
        ...
        check("bundle-has-raw-logs", os.path.isfile(os.path.join(evidence_dir, "raw", "run_001.stderr")))
...
def main() -> int:
    ...
    parser.add_argument("--apply", action="store_true", help="insert into docs/PERF_LEDGER.md")
    args = parser.parse_args()

    if args.self_test:
        return _self_test()
    required = ("focr_stages", "ref_stages", "roofline", "claim_id", "fixture_hash",
                "arch_features", "correctness_proof")
