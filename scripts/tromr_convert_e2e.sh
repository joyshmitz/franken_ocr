...
#   1/5 GATE        zoo export present (model.safetensors + the 4 tokenizer
#                   JSONs) else SKIP-with-SUCCESS
#   2/5 BIN         build (or take FOCR_BIN) — release profile
#   3/5 NEGATIVE    convert /nonexistent => exit 3 (ModelNotFound);
#                   --model-id not-a-model => exit 2 (Usage)
#   4/5 CONVERT     model.safetensors -> tromr.focrq; JSON must report
#                   status=ok, model_id=tromr, tensors=260,
#                   tensors_quantized=0 (the default-f32 policy, spec §11)
#   5/5 DETERMINISM re-convert to a scratch path => byte-identical artifact
#
