//! Pillow-bit-exact **BICUBIC** resampler — the `FOCR_RESAMPLE=pil-bicubic`
//! reference path for every preprocess resize site (bd-30me, DISC-001).
//!
...
/// Bicubic filter support radius (`Resample.c`: `{bicubic_filter, 2.0}`).
...
fn precompute_coeffs(in_size: u32, out_size: u32) -> PassCoeffs {
    ...
    for xx in 0..out_size as usize {
        ...
        for (x, slot) in k.iter_mut().enumerate().take(taps) {
            ...
            let w = bicubic_filter(((x + xmin) as f64 - center + 0.5) * ss);
            *slot = w;
            ww += w;
        }
        if ww != 0.0 {
...
fn resample_horizontal(src: &RgbImage, out_w: u32) -> RgbImage {
    let coeffs = precompute_coeffs(src.width(), out_w);
...
fn resample_vertical(src: &RgbImage, out_h: u32) -> RgbImage {
    let coeffs = precompute_coeffs(src.height(), out_h);
...
pub fn resize_bicubic(src: &RgbImage, out_w: u32, out_h: u32) -> RgbImage {
    assert!(
        out_w > 0 && out_h > 0,
        "pil_resample::resize_bicubic: zero output dimension {out_w}x{out_h}"
    );
    ...
    match (need_horizontal, need_vertical) {
        ...
        (true, false) => resample_horizontal(src, out_w),
        (false, true) => resample_vertical(src, out_h),
        (true, true) => resample_vertical(&resample_horizontal(src, out_w), out_h),
    }
}
...
mod tests {
    ...
    fn zero_output_dimension_panics() {
        ...
        let msg = panic
            ...
            .unwrap_or_default();
        assert!(msg.contains("resize_bicubic"), "unexpected panic: {msg}");
    }
}
