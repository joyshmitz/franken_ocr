...
| **G2** | **CPU speed beats the reference stack — measured honestly, per stage** | **Decode-per-token** wall-clock on the same CPU **faster** than the PyTorch/HF + `transformers` CPU reference (the defensible int8/int4 win). **Vision-prefill** measured and reported honestly — *parity-or-slower is acceptable in the f32 v1* (frankentorch f32 matmul vs MKL/oneDNN; see §9.3, §11). **End-to-end-faster is a post-int4 / optional-int8-vision STRETCH, not a v1 hard gate.** All ratios are honest best-of-N with thread/allocator/precision fairness controls (§9.3). | §9 performance |
...
- **Not** leaderboard SOTA. Unlimited-OCR sits ~93.9% on OmniDocBench v1.6, **behind** PaddleOCR-VL 1.6 (96.33) and MinerU 2.5-Pro (95.69) **[REPORTED]**. Our pitch is *fidelity to this model* + *constant-memory long-document parsing on CPU* + *speed*, not beating the benchmark.
...
- **Pre-allocate everything** — R-SWA's KV cache is `O(m + 128)` *constant*, so we allocate one fixed ring buffer per layer and never realloc, even for a 40-page parse. No growth logic, trivially memory-safe.
...
- **⚠️ The honest performance state, and franken_ocr's actual wedge.** frankentorch's int8 path today is a **per-output-element SDOT/VNNI dot product** (no matrix-tile kernel, no weight pre-packing). On M4 it measured (per-doc forward, all cores) ~**14.8 / 34.3 / 64.0 ms** at seq 128/256/512 — **still ~1.5–2.4× behind ONNX-int8/MLAS** (7.6 / 14.5 / 41 ms). frankentorch's own diagnosis: SDOT only ~doubled over autovectorized scalar, and **per-element `target_feature` dispatch + horizontal reduction dominate**. **frankentorch does NOT yet have a tiled `i8mm`/SMMLA kernel** — it is *named* as the next lever but **unbuilt**; **int4 exists nowhere; there is no GPU.** Closing that gap with a **tiled, register-blocked SMMLA (ARM i8mm, 32 MACs/instr) + AVX-512-VNNI/AMX GEMM that amortizes loads and the int32 reduction across a register tile, fed by offline pre-packed weights** — plus **int4-group** — is **precisely `franken_ocr`'s reason to exist** (§6.2–§6.4). We reuse the existing SDOT/VNNI dot as the bit-exact int8 baseline/oracle and **build** the tiled GEMM as our contribution, holding ourselves to the honest head-to-head bar (§9.3), not a self-relative one.
...
- **Streaming results** (per-page text as it completes) to the robot/NDJSON consumer use **`std::sync::mpsc` + `std::thread`** (proven, simple), main looping on `recv_timeout(~40ms)` until the worker finishes.
...
Vision encode is a **fixed per-page cost** (run once, ~256 out tokens, dominated by SAM/CLIP attention+conv GEMMs over ~4096 patches). Decode is a **per-output-token cost** (memory-bound expert GEMV + 129K-vocab logits). These two cost centers are optimized **separately** (§6, §9).
...
[3] QUANTIZE (the recipe from §2.6, validated by NVFP4/GGUF prior art)
    ...
    KEEP HIGH PRECISION (BF16 verbatim / F32) — DO NOT QUANTIZE:
       entire vision tower (sam_model.* + vision_model.*), projector, embed_tokens, lm_head*, MoE router gate, ALL norms
       (*lm_head: int8 is the highest-value single GEMM; quantize it but gate behind a measured-accuracy kill-switch)
...
- **Two parallelism axes.** *Latency* (one page, faster): parallelize **within** a forward — prefill GEMMs over output-row blocks, MoE over experts, attention over heads. *Throughput* (many pages/docs): parallelize **across** pages (one sequential forward per worker, N workers), which scales near-linearly and is the right mode for batch document processing. `focr` exposes both (`--page-parallelism` vs single-doc).
...
- **f16/bf16 bulk dequant:** the high-precision (vision/embeddings) tensors are f16 on disk; bulk-convert to f32 with SIMD (`half::convert_to_f32_slice`; aarch64 `fp16` / x86 `f16c`) — not per-element.

### 6.13 Build-time optimization (free, model-agnostic, do them all)

- **LTO `fat` + `codegen-units=1` + `panic=abort`** on the shipping `release` profile (already set); keep the separate `release-perf` profile (frame pointers, `lto=thin`, `strip=false`) for profiling only — never profile the size/abort build (profiling-software-performance build-flags rule).
...
- `robot schema` **self-describes** all event types (machine-readable contract); a `robot_contract_tests.rs` validates emitted events against a frozen JSON schema fixture.
- **Streaming**: worker `std::thread` runs the engine with an `mpsc::Sender`; main loops `recv_timeout(~40ms)` emitting events until `worker.is_finished()`, then drains and `join()`s.
...
- **Oracle wiring (ML-System-class):** PyO3 in-process bridge to the pinned reference with **`torch.use_deterministic_algorithms(True)`**, seeded RNG captured per call, and a **per-op ULP tolerance table** (default **4 ULP f32 matmul, 2 ULP elementwise**) as the L1/L2 comparator — *not* a hand-guessed epsilon. `EngineIdentity::{Subject, Oracle}` asserted-distinct so the oracle is never compared against itself.
...
`benches/gauntlet` shells out to the **real CPU PyTorch/HF reference** (`FOCR_REFERENCE_PYTHON` env) per stage, parses elapsed, reports the **honest focr/reference ratio** (tag each row OK/warn/slower/"focr faster") — not a self-relative number. This is how G2 is proved, and it is only meaningful with **apples-to-apples fairness controls, all mandatory**:
...
- **Native-resolution token packing** — pack the deterministic per-page token budget (256 base / 100/crop) into fixed contiguous buffers so prefill GEMMs are remainder-free.
...
- **Goals**: the int8 GEMMs run on native intrinsics across arches; **the DECODE path is demonstrated faster than the CPU reference** (the defensible part of G2; end-to-end-faster is a later stretch, §1.1).
...
- **Exit gates**: bit-identical to scalar (integer accumulation); **gauntlet per-stage ratios recorded with the fairness controls of §9.3, and the decode-per-token path is faster than the CPU reference** on the primary arches (vision-prefill parity-or-slower in f32 is acceptable and recorded honestly); every landed lever has a `NEGATIVE_EVIDENCE.md`/`PERF_LEDGER.md` entry; parity still green.
...
| **Vision int8 trap** (ViT activation outliers) | MED | Keep vision tower f16/f32 in v1 (validated by both prior quants); int8 vision is a later isolated experiment only after decoder parity. |
...
| **End-to-end may not beat the CPU reference in v1** (f32 vision tower vs MKL/oneDNN; frankentorch's own profiling tags it slower on some ops) | MED | **G2 reframed (§1.1) to per-stage honesty**: target the *decode* win (int8/int4), report vision-prefill parity-or-slower openly; "end-to-end faster" is a post-int4 / optional-int8-vision stretch, not a v1 gate. The unbuilt tiled SMMLA/VNNI GEMM (§3.2, §6.2) is the lever that narrows it. |
...
- Gauntlet: **decode-per-token faster than the CPU PyTorch/HF reference** on the primary arches (the gating part); **vision-prefill ratio recorded honestly** (parity-or-slower acceptable in f32 v1); end-to-end-faster is a tracked *stretch*. Honest best-of-N with thread/allocator/precision fairness (§9.3) in `PERF_LEDGER.md`.
- Constant decode-side **memory** across page count (R-SWA property preserved): memory flat for a 40-page parse. (Per-token *compute* still grows with the reference block `m`, §6.4 — not claimed constant.)
